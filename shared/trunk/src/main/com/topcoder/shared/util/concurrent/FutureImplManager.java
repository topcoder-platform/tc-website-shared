...
public class FutureImplManager<ID, T> {
    ...
    private Map<ID, TimedEntry<WeakReference<ManagedFutureImpl<T, ID>>>> pendingFutures = new HashMap();
    ...
    public ManagedFutureImpl<T, ID> newFuture(ID id) {
        ...
        ManagedFutureImpl<T, ID> future = new ManagedFutureImpl(id, futureReleaseHandler);
        synchronized (requestResponseLock) {
